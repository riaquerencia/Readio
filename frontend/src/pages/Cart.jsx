...
const Cart = () => {
  ...
  useEffect(()=>{
    const fetch= async ()=>{
      const res = await axios.get("https://readio-wrte.onrender.com/api/v1/get-user-cart",{headers}
  ...
  const deleteItem = async (bookId) => {
     const response = await axios.put(`https://readio-wrte.onrender.com/api/v1/remove-from-cart/${bookId}`,{},{headers}
     );
  ...
  const PlaceOrder = async () =>{
    try{
      const res = await axios.post(
        `https://readio-wrte.onrender.com/api/v1/place-order`,{order:Cart},
        {headers}
      );
          ...
          {Cart.map((items, i) => (
            <div className='w-full my-4 rounded flex flex-col md:flex-row p-4 bg-zinc-800 justify-between items-center' key={i}>
              ...
              <div className='flex mt-4 w-full md:w-auto items-center justify-between'>
                ...
                <button className='bg-red-100 text-red-700 border-red-700 rounded p-2 ms-12' onClick={() => deleteItem(items._id)}>
                  <AiFillDelete />
                </button>
              </div>
            </div>
